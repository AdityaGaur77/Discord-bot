...
class FTCApiCog(commands.Cog):
    ...
    async def ftc_team(self, interaction: discord.Interaction, team_number: int):
        ...
        location_parts = [team.get("city"), team.get("stateProv"), team.get("country")]
        ...
        embed.set_footer(text=f"Season {season} | Data via ftcscout.org/rest/v1")
        await interaction.followup.send(embed=embed)

    # /ftc teamevents

    @ftc_group.command(name="teamevents", description="Show events a team competed in this season")
    @app_commands.describe(
        team_number="FTC team number",
        season="Season year (optional, defaults to configured season)",
    )
    async def ftc_teamevents(
        self, interaction: discord.Interaction, team_number: int, season: int = None
    ):
        await interaction.response.defer(thinking=True)
        cfg = await self.bot.db.get_config(interaction.guild_id)
        s = season or _season(cfg)

        events = await ftc.get_team_events(team_number, s)
        if events is None:
            await interaction.followup.send(f"❌ Team **{team_number}** not found.")
            return
        if not events:
            await interaction.followup.send(
                f"No events found for team **{team_number}** in season **{s}**."
            )
            return
        ...
        for e in events:
            ...
            lines.append(f"**{code}** — {name}{rank_str} ({w}W / {l}L)")

        embed = discord.Embed(
            title=f"Events — Team {team_number} | Season {s}",
            description="\n".join(lines),
            color=0x1565C0,
        )
    ...
    @ftc_group.command(name="teamawards", description="Show awards won by a team this season")
    @app_commands.describe(team_number="FTC team number")
    async def ftc_teamawards(self, interaction: discord.Interaction, team_number: int):
        await interaction.response.defer(thinking=True)
        cfg = await self.bot.db.get_config(interaction.guild_id)
        season = _season(cfg)

        awards = await ftc.get_team_awards(team_number, season)
        if awards is None:
            await interaction.followup.send(f"❌ Team **{team_number}** not found.")
            return
        if not awards:
            await interaction.followup.send(
                f"No awards found for team **{team_number}** in season **{season}**."
            )
            return

        lines = []
        for a in awards[:20]:
            name       = a.get("name") or (a.get("award") or {}).get("name", "Unknown Award")
            event      = a.get("event") or {}
            event_name = event.get("name") or event.get("code") or "?"
            person     = a.get("person", "")
            suffix     = f" ({person})" if person else ""
            lines.append(f"🏅 **{name}**{suffix} — {event_name}")

        embed = discord.Embed(
            title=f"Awards — Team {team_number} | Season {season}",
            description="\n".join(lines),
            color=0xFB8C00,
        )
        embed.set_footer(text="Data via ftcscout.org/rest/v1")
