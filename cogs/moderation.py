...
# Pre-compile all built-in patterns once at import time
_PATTERN_CACHE: dict[str, re.Pattern] = {
    word: re.compile(r"\b" + re.escape(word) + r"\b", re.IGNORECASE)
    for word in BUILTIN_PROFANITY
}

# How long to wait before re-sending a DM to the same user (seconds)
_DM_COOLDOWN = 15
...
def _find_violation(text: str, words: list[str]) -> str | None:
    ...
    lower = text.lower()
    for word, pattern in _PATTERN_CACHE.items():
...
class ModerationCog(commands.Cog):
    def __init__(self, bot):
        ...
        # user_id -> monotonic timestamp of last DM sent
        self._dm_cooldown: dict[int, float] = {}
    ...
    async def _get_config(self, guild_id: int) -> dict:
        if guild_id not in self._config_cache:
            ...
            self._config_cache[guild_id] = cfg or {}
        return self._config_cache[guild_id]

    async def _side_effects(self, message: discord.Message, matched: str):
        """DM the user and post to the mod log. Runs as a background task."""
        now = time.monotonic()
        last_dm = self._dm_cooldown.get(message.author.id, 0)
        if now - last_dm >= _DM_COOLDOWN:
            self._dm_cooldown[message.author.id] = now
            try:
    ...
    async def on_message(self, message: discord.Message):
        ...
        # DM + mod log run in the background so they never slow down deletion
        asyncio.create_task(self._side_effects(message, matched))
    ...
    # Invalidate config cache when channels are reconfigured
