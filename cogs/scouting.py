...
class ScoutingCog(commands.Cog):
    ...
    @scout_group.command(name="top", description="Show top teams by average scouted score")
    @app_commands.describe(event="Filter by event code (optional)")
    async def scout_top(self, interaction: discord.Interaction, event: str = None):
        event_code = event.upper() if event else None
        reports    = await self.bot.db.get_all_scouts(interaction.guild_id, event_code)

        if not reports:
            await interaction.response.send_message(
                "No scouting data yet. Use `/scout add` to submit reports.", ephemeral=True
            )
            return
        ...
        for r in reports:
            ...
            teams[r["team_number"]].append(total)

        ranked = sorted(
            [(team, sum(scores) / len(scores), len(scores)) for team, scores in teams.items()],
            key=lambda x: x[1],
            reverse=True,
        )

        lines = [
            f"**#{i}** Team **{team}** — avg **{avg:.1f}** ({count} report{'s' if count != 1 else ''})"
            for i, (team, avg, count) in enumerate(ranked[:15], 1)
        ]

        title = f"Top Scouted Teams{f' at {event_code}' if event_code else ''}"
        embed = discord.Embed(title=title, description="\n".join(lines), color=0x43A047)
        embed.set_footer(text=f"{len(ranked)} team(s) scouted total")
        await interaction.response.send_message(embed=embed)
